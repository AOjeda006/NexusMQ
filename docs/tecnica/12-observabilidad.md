...
`…_request_bytes_total`, `…_messages_total`, `…_request_duration_seconds`) llevan dos etiquetas: `api`
...
protocolo aunque compartan el mismo broker y las mismas particiones. `…_messages_total` cuenta
**records** (mensajes), distinto de `…_requests_total`: un request agrupa N records en un batch.
La **saturación** de conexiones se ve en el gauge `nexus_broker_connections_active{plane}` (planos
`native`/`kafka`/`admin`), cuyo recuento va atado por RAII al ciclo de vida de cada conexión para no
fugarse ante cierres abruptos ([ADR-0039](../adr/adr-0039-gauge-conexiones-activas-raii.md)).

> **Los *nombres* de todas las métricas** (con tipo, etiquetas, unidad y HELP) viven en el catálogo
> as-built [`docs/metrics.md`](../metrics.md), **fuente de verdad única** contra la que codifica la
> consola. `openapi.yaml` fija la *forma* del snapshot; el catálogo, los *nombres*. Añadir o
> renombrar una métrica obliga a actualizar el catálogo en el mismo commit.
