...
Este ADR **refina el desglose** (no cambia el diseño del protocolo): el desglose detallado ubicaba `FrameReader`/`FrameWriter` en `protocol/frame.hpp`, pero el grafo de dependencias fija `nexus-protocol → common` y «protocolo puro (encode/decode, sin E/S ni async)». La decisión resuelve ese conflicto entre dos fuentes de verdad.
...
- (−) Un target más en el árbol (15 en total) y una desviación respecto a la ubicación del desglose detallado (anotada en la hoja de ruta).
